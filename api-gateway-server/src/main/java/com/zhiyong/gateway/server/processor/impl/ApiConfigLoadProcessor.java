 ...
 **/
@Component
public class ApiConfigLoadProcessor implements ApiProcessor {
    ...
    public String getName() {
        return ApiConfigLoadProcessor.class.getName();
    }

    @Override
    public void run(ApiContext context) throws GatewayException {
        ApiRequest apiRequest = context.getApiRequest();
        AppCfg appCfg = apiConfigCache.getAppCfg(apiRequest.getAppKey());
        AssertUtil.notNull(RequestHeaderConstant.APP_KEY + ":" + apiRequest.getAppKey(), appCfg);
